...
class DeepActorCriticAgent(mp.Process):
    def __init__(self, id, env_name, agent_params, shared_state, env_params):
        ...
        # self.weather_ratio = os.path.join("./gradients/weather_ratio.pth")
        self.future = self.params['future']
    ...
    def gradient_update(self):
        # print("update_step",self.step_label)
        for _ in range(int(self.gradient_size*self.ratio)):
            self._gradient_update(1)
            
        for _ in range(self.gradient_size-int(self.gradient_size*self.ratio)):
            self._gradient_update(0)
    ...
    def run(self):
        ...
        if len(self.state_shape) == 3:  # Screen image is the input to the agent
            if self.continuous_action_space:
                ...
                self.policy_dy = DeepDiscreteActor(self.state_shape, self.action_shape, device).to(device)
        ...
        if self.actor_name == "global":
            ...
            if self.params['load_trained_model']:
                try:
                    ...
                    model_file_name = "tool_models/model_dy/Async-A2C_Carla-v0.ptm"
                    agent_state = torch.load(model_file_name, map_location= lambda storage, loc: storage)
                    self.policy_dy.load_state_dict(agent_state["Actor"])
                    self.policy_dy.to(device)
            ...
            # weather_ratio = 0
            if self.future:
                self.future_data=torch.load("gradients/weather_ratio.pth", map_location=device)

        for episode in range(self.params["max_num_episodes"]):
            if self.actor_name == "global" and self.global_step_num != 0:
               writer.add_scalar(self.actor_name + "/accuracy", self.accracy/(self.step_num+1), episode)
            ...
            if args.test and episode != 0:
                ...
                writer.add_scalar(self.actor_name + "/mean_accuracy", self.accracy_sum/episode, episode)
            self.accracy = 0
            obs = self.env.reset()
            done = False
            ep_reward = 0.0
            ...
            while not done:
                ...
                self.ratio += self.selected
                action = self.get_action(obs)
                next_obs, reward, done, py_measurement, weather_num = self.env.step(action)
                
                if self.step_num < len(speed):
                    speed[self.step_num] = (speed[self.step_num]*speed_episode[self.step_num]+py_measurement["forward_speed"])/(speed_episode[self.step_num]+1)
                    speed_episode[self.step_num] += 1
                else:
                    speed.append(py_measurement["forward_speed"])
                    speed_episode.append(0)
                    
                speed_step.append(py_measurement["forward_speed"])
                # if self.step_num < len(speed):
                #     speed[self.step_num] = (speed[self.step_num]*episode+py_measurement["forward_speed"])/(episode+1)
                #     speed_episode[self.step_num] += 1
                # else:
                #     speed.append(py_measurement["forward_speed"])
                #     speed_episode.append(0)
                ...
                # weather_ratio += np.argmax(weather_num)
                ...
                step_num += 1
                self.step_num += 1
                if step_num >= self.params["learning_step_thresh"]:
                    if self.single_update:
                        self.policy_dy.load_state_dict(agent_state["Actor"])
                        self.policy_dy.to(device)
                    if self.future:
                        if self.step_label+1<len(self.future_data):
                            self.ratio = self.future_data[self.step_label+1]
                            # print("ratio_step", self.step_label)
                            # print("ratio",self.ratio)
                    else:
                        self.ratio = self.ratio/step_num
                    self.gradient_update()
                    # print(weather_ratio/step_num)
                    # weather_pattern.append(weather_ratio/step_num)
                    self.ratio = 0
                    step_num = 0
                    self.step_label += 1
                    # weather_ratio = 0
                ...
                if done and self.actor_name == "global" and args.test:
                        episode_rewards.append(ep_reward)
                        writer.add_scalar(self.actor_name + "/cloudy_proportion", cloudy/self.step_num, episode)
                        speed_matrix.append(speed_step)
            ...
            if self.actor_name == "global":
                if self.dummy_gradient != 1:
                    if not os.path.exists(self.grad_filename):
                        torch.save([self.dummy_gradient], self.grad_filename)
                    else:
                        old_data=torch.load(self.grad_filename)
                        old_data.append(self.dummy_gradient)
                        torch.save(old_data, self.grad_filename)
                    self.dummy_gradient = 0
                # if not os.path.exists(self.weather_ratio):
                #         torch.save(weather_pattern, self.weather_ratio)
                ...
                writer.add_scalar(self.actor_name + "/ep_reward", ep_reward, episode)
                for i in range(len(speed)):
                    writer.add_scalar(self.actor_name + "/avg_stepspeed", speed[i], i)
                if args.test:
                    writer.add_scalar(self.actor_name + "/mean_ep_reward", np.mean(episode_rewards), episode)
                    writer.add_scalar(self.actor_name + "/intersection_offroad", py_measurement["intersection_offroad"], episode)
                    writer.add_scalar(self.actor_name + "/intersection_otherlane", py_measurement["intersection_otherlane"], episode)
                    intersection_offroad.append(py_measurement["intersection_offroad"])
                    intersection_otherlane.append(py_measurement["intersection_otherlane"])
                    writer.add_scalar(self.actor_name + "/mean_inter_offroad", np.mean(intersection_offroad), episode)
                    writer.add_scalar(self.actor_name + "/mean_inter_otherlane", np.mean(intersection_otherlane), episode)
                    
        for i in speed_matrix:
            while(len(i)<max((len(i) for i in speed_matrix))):
                i.append(0)
        np.savetxt('./speed_collection/GF5.csv', np.array(speed_matrix), delimiter = ',')
