...
DISCRETE_ACTIONS = {
    ...
    # Sharp Left  
    11: [0.75, -1.0],
    # SSharp Right & Decelerate  
    12: [-0.5, 1.0],
    # SSharp Left  & Decelerate  
    13: [-0.5, -1.0],
    # Sharp Right & Bear accelerate 
    14: [0.5, 0.75],
    # Sharp Left  & Bear accelerate
    15: [0.5, -0.75],
    # Forward  
    16: [0.75, 0.0],
    # Emergency Brake 
...
class Storm(object):
    def __init__(self, precipitation):
        ...
        if self._t == -150.0:
            ...
            self._increasing = random.choice([True, False])
        self.clouds = 0.0
        self.rain = 0.0
        self.puddles = 0.0
        self.wind = 0.0
    ...
    def tick(self, delta_seconds):
        ...
        self._t = clamp(delta + self._t, -150.0, 100.0)
...
class MultiCarlaEnv(*MultiAgentEnvBases):
    def __init__(self, configs=None):
        ...
        self.all_id = []
        self.step_num = 0
        self.walkers_run_list = []
        self.walkers_speed_list = []
        self.stop_time = 0
        self._dynamic_weather = None
        self._weather_conditions = [[80.0, 0.0, 0.0, 0.3499999940395355],
                                    [80.0, 30.0, 50.0, 0.4000000059604645]]
        self.dynamic_on = self._env_config["dynamic_on"]
    ...
    def _init_server(self):
        ...
        # while(not(45000<self._server_port and self._server_port<60000)):
        #     self._server_port = MultiCarlaEnv._get_free_tcp_port()
            # print(self._server_port)
        # self._server_port = random.randint(30000, 60000)
        ...
        if self._render and self._env_config.get("spectator_loc"):
            ...
            spectator.set_transform(
                carla.Transform(location,
                                carla.Rotation(yaw=180 + angle, pitch=-25)))
    ...
    def _clean_world(self):
        ...
        #    self._client.apply_batch([carla.command.DestroyActor(x) for x in self.walkers_list])
    ...
    def calculate_weather_prob(self):
        Dist = []
        for i in range(len(self._weather_conditions)):
            Dist.append(torch.norm(torch.Tensor(self._weather)-torch.Tensor(self._weather_conditions[i])))
        Prob = [(Dist[1]/sum(Dist)).item(), (Dist[0]/sum(Dist)).item()]
        return Prob
    ...
    def _spawn_new_agent(self, actor_id):
        ...
        for retry in range(RETRIES_ON_ERROR - 1):
            ...
            print("spawn_actor: Retry#:{}/{}".format(retry + 1,
                                                     RETRIES_ON_ERROR))
            # if self.npc_list != []:
            #     self._client.apply_batch([carla.command.DestroyActor(x.id) for x in self.npc_list])
            #     self.npc_list = []
        if vehicle is None:
    ...
    def _spawn_new_vehicle(self):
        if self.npc_list != []:
           self._client.apply_batch([carla.command.DestroyActor(x) for x in self.npc_list])
           self.npc_list = []
        # -----------------------------------NPC-----------------------------------------------
        # self.npc_list = []
    ...
    def _pedestrians_walking(self, walker, speed):
    ...
    def _spawn_new_pedestrians(self):
        ...
        if random.random()>=0.3:
            ...
            vehicle = self.world.try_spawn_actor(blueprint, transform)
            if vehicle is not None:
                self.walkers_list.append(vehicle)
                self.walkers_run_list.append(random.random()*250)
                self.walkers_speed_list.append(random.random())
        #----------------
        ...
        if random.random()>=0.3:
            ...
            if vehicle is not None:
                self.walkers_list.append(vehicle)
                self.walkers_run_list.append(random.random()*250+250)
                self.walkers_speed_list.append(random.random())
        ...
        if random.random()>=0.4:
            ...
            if vehicle is not None:
                self.walkers_list.append(vehicle)
                self.walkers_run_list.append(random.random()*250+500)
                self.walkers_speed_list.append(random.random())
    ...
    def _reset(self):
        ...
        self.elapsed_time = 0.0
        # for i in range(14):
        #     self.world.set_weather(WEATHERS[i])
        ...
        #     ]
        #     print("Weather: {}, Conditions: {}".format(i,
        #                                                self._weather))
        ...
        self.temp_weather_num = weather_num
        ...
        #------------for dynamic weather-----------------------------
        if self.dynamic_on:
            self._dynamic_weather = Weather(self.world.get_weather())
            self.temp_weather_num = self.calculate_weather_prob()
        #------------for dynamic weather-----------------------------
        
        
        # print("Weather: {}".format(self.temp_weather_num))
        # if self.npc_list != []:
        #    self._client.apply_batch([carla.command.DestroyActor(x.id) for x in self.npc_list])
        #    self.npc_list = []
        ...
        for actor_id, actor_config in self._actor_configs.items():
            ...
            if self._done_dict.get(actor_id, False) is True:
                ...
                # print("image_dir",self._actors[actor_id])
                ...
                print("Actor: {} start_pos_xyz(coordID): {} ({}), "
                      "end_pos_xyz(coordID) {} ({}), weather: {}, port: {}".format(
                          actor_id,
    ...
    def dynamic_weather(self, speed = 5):
        speed_factor = speed
        update_freq = 0.1 / speed_factor

        self.elapsed_time += self._fixed_delta_seconds
        if self.elapsed_time > update_freq and (not self._dynamic_weather is None):
            self._dynamic_weather.tick(speed_factor * self.elapsed_time)
            self.world.set_weather(self._dynamic_weather.weather)
            # print("Storm_t:", self._dynamic_weather._storm._t)
            # sys.stdout.write('\r' + str(self._dynamic_weather) + 12 * ' ')
            # sys.stdout.flush()
            self.elapsed_time = 0.0
    ...
    def step(self, action_dict):
        ...
        try:
            ...
            for actor_id, action in action_dict.items():
                self.last_obs_dict[actor_id] = self._last_obs
                self.last_reward_dict[actor_id] = 0
                # self.last_info_dict[actor_id] = None
            ...
            return obs_dict, reward_dict, self._done_dict, info_dict, self.weather_num
        except Exception:
            ...
            self._clean_world()
            self._clear_server_state()
            return self.last_obs_dict, self.last_reward_dict, self._done_dict, self.last_info_dict, self.weather_num
    ...
    def _step(self, actor_id, action):
        ...
        #------------for dynamic weather-----------------------------
        if self.dynamic_on:
            self.dynamic_weather()
        #------------for dynamic weather-----------------------------
        
        self.weather_num = self.temp_weather_num
        # if self._num_steps[actor_id] >= self.change_weather:
        #     self.change_weather += random.randint(1,5)
        #     weather_num = random.choice([0,4])
        #     self.world.set_weather(WEATHERS[weather_num])
        #     self.temp_weather_num = weather_num
        ...
        if self.dynamic_on:
            ...
            self.temp_weather_num = self.calculate_weather_prob()
        # print("Weather: {}, \n Weather Prob: {}".format(self._weather, self.temp_weather_num))
        #------------for dynamic weather-----------------------------
        
        # print("walker_list", len(self.walkers_list))
        # print("walker_trigger",self.walkers_run_list)
        if self.walkers_list != []:
           for run_i,x in enumerate(self.walkers_list):
            #    print("walker_trigger",self.walkers_run_list[run_i]*300+run_i*200)
               if self.walkers_run_list[run_i] <= self._num_steps[actor_id]:
                  self._pedestrians_walking(x, (self.walkers_speed_list[run_i]*2+1))
        ...
        if config["manual_control"]:
            ...
            if getattr(self._actors[actor_id], "set_autopilot", 0):
                self._actors[actor_id].set_autopilot(True)
        else:
        ...
        cmpt_reward = Reward()
        reward, self.stop_time = cmpt_reward.compute_reward(self._prev_measurement[actor_id],
                                            py_measurements, flag, self.stop_time)
