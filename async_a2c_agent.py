...
class DeepActorCriticAgent(mp.Process):
    ...
    def run(self):
        ...
        # a = DeepActor(self.state_shape, self.action_shape, device).to(device)
        # summary(a, (6, 84, 84))
        # c = DeepCritic(self.state_shape, self.critic_shape, device).to(device)
        # summary(c, (6, 84, 84))
        ...
        for episode in range(self.params["max_num_episodes"]):
            ...
            while not done:
                ...
                next_obs, reward, done, py_measurement, _ = self.env.step(action)
                ...
                # print("speed: ",py_measurement["forward_speed"])
        ...
        for i in speed_matrix:
            while(len(i)<max((len(i) for i in speed_matrix))):
                i.append(0)
        np.savetxt('./speed_collection/dy3.csv', np.array(speed_matrix), delimiter = ',')
