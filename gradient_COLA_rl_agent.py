...
parser.add_argument("--params-file", help="Path to the parameters file. Default= ./async_a2c_parameters.json",
                    default="gradient_COLA_rl_parameters.json", metavar="async_a2c_parameters.json")
parser.add_argument("--model-dir", default="tool_models/model_dy/", metavar="MODEL_DIR",
...
class DeepActorCriticAgent(mp.Process):
    def __init__(self, id, env_name, agent_params, shared_state, env_params):
        ...
        self.grad_filename = os.path.join("./gradients/grads_dy_episode.pth")
    ...
    def run(self):
        ...
        for episode in range(self.params["max_num_episodes"]):
            ...
            if self.actor_name == "global":
                if self.episode_grads != []:
                    if not os.path.exists(self.grad_filename):
                        torch.save([self.episode_grads], self.grad_filename)
                    else:
                        old_data=torch.load(self.grad_filename)
                        old_data.append(self.episode_grads)
